...
def gdelt(requete, journal, maxi=60):
    ...
    try:
        ...
        journal.append({"source": "GDELT", "articles": 0, "erreur": str(exc)[:70]})
        return []

    if r.status_code == 429:
        # GDELT limite par adresse IP. Les serveurs GitHub sont partages entre des
        # milliers d'utilisateurs : le quota peut deja etre consomme par d'autres.
        # Ce n'est pas rattrapable, et ce n'est pas une panne de ton cote.
        journal.append({"source": "GDELT", "articles": 0,
                        "erreur": "quota partage GitHub epuise"})
        return []
    if r.status_code != 200:
        journal.append({"source": "GDELT", "articles": 0,
                        "erreur": f"code {r.status_code}"})
        return []

    # Une reponse 200 est un succes. Mon ancien test exigeait que le corps commence
    # par une accolade et rejetait donc des reponses valides precedees d'espaces,
    # ou vides quand la requete ne ramene simplement rien.
    corps = r.text.strip()
    if not corps:
        journal.append({"source": "GDELT", "articles": 0, "erreur": None})
        return []
    try:
        articles = r.json().get("articles", []) or []
    except ValueError:
        journal.append({"source": "GDELT", "articles": 0,
                        "erreur": "reponse non JSON (service indisponible)"})
        return []
...
def rss(urls, journal):
    ...
    for u in urls:
        ...
        google = "news.google.com" in domaine
        for e in flux.entries[:40]:
            ...
            media = ""
            if google:
                titre, media = separer_media(titre)
                # feedparser expose parfois le media directement
                if not media:
                    media = (getattr(getattr(e, "source", None), "title", "") or "")
            ...
            sortie.append({
                "titre": titre,
                "url": getattr(e, "link", ""),
                "source": (media or domaine).lower(),
                "agence": est_agence(media),
                "date": iso,
                "origine": "rss",
...
AGENCES = ("reuters", "associated press", " ap ", "afp", "bloomberg",
           "agence france", "pa media", "dpa")


def separer_media(titre):
    """Google News termine chaque titre par " - Nom du media". On le detache.

    Deux gains d'un coup : le titre nettoye se regroupe bien mieux, et on recupere
    enfin la rediction reelle. Sans ca, tous les articles etaient attribues a
    "news.google.com" et le compteur de sources independantes valait 1 partout.
    """
    if " - " not in titre:
        return titre.strip(), ""
    tete, _, media = titre.rpartition(" - ")
    # Un vrai nom de media est court ; au-dela c'est un morceau de titre.
    if len(media) > 45 or len(tete) < 12:
        return titre.strip(), ""
    return tete.strip(), media.strip()


def est_agence(media):
    """Une depeche d'agence reprise par vingt sites reste UNE source."""
    bas = f" {media.lower()} "
    return any(a in bas for a in AGENCES)
...
def proches(a, b, courants=frozenset()):
    """Deux titres decrivent le meme evenement.

    Le critere decisif n'est pas le NOMBRE de mots partages mais leur RARETE.
    Deux depeches sans rapport partagent facilement "marche", "prix", "hausse" :
    ce sont des mots de remplissage. En revanche "Ormuz", "ASML" ou "lithographie"
    en commun, c'est le meme sujet. On ecarte donc les mots trop frequents dans
    la collecte du jour avant de comparer.
    """
    inter = a & b
    if len(inter) < 3:
        return False
    rares = inter - courants
    if len(rares) < 2:
        return False
    if len(inter) / len(a | b) >= config.SEUIL_REGROUPEMENT:
        return True
    return len(inter) >= 4 and len(inter) / min(len(a), len(b)) >= 0.55


def mots_courants(sacs, seuil=0.12):
    """Mots presents dans plus de 12 % des titres : trop banals pour prouver
    quoi que ce soit."""
    if len(sacs) < 20:
        return frozenset()
    freq = {}
    ...
    limite = max(3, seuil * len(sacs))
    return frozenset(m for m, n in freq.items() if n > limite)
...
def regrouper(articles):
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            if sacs[j] and proches(sacs[i], sacs[j], courants):
    ...
    for membres in paquets.values():
        ...
        redactions = {m["source"] for m in membres if m["source"]}
        # Toutes les depeches d'agence d'un dossier comptent pour UNE source :
        # vingt sites qui republient Reuters ne sont pas vingt confirmations.
        agences = {m["source"] for m in membres if m.get("agence") and m["source"]}
        independantes = len(redactions - agences) + (1 if agences else 0)
        dossiers.append({
            "titre": membres[0]["titre"],
            "url": membres[0]["url"],
            "reprises": len(membres),
            "sources_independantes": independantes,
            "sources": sorted(redactions)[:8],
            "via_agence": bool(agences),
...
def cours(symbole, journal):
    """Clotures quotidiennes via Yahoo Finance. Gratuit, sans cle, a J-1.

    Stooq a rendu zero sur huit symboles lors de deux executions : il refuse les
    requetes venant des serveurs GitHub. Yahoo les accepte, a condition d'envoyer
    un en-tete de navigateur.
    """
    url = (f"https://query1.finance.yahoo.com/v8/finance/chart/"
           f"{urllib.parse.quote(symbole)}?range=6mo&interval=1d")
    ...
    try:
        ...
        if r.status_code != 200:
            journal.append({"source": f"Yahoo {symbole}", "articles": 0,
                            "erreur": f"code {r.status_code}"})
            return []
        bloc = (r.json().get("chart", {}).get("result") or [{}])[0]
    except Exception as exc:
        journal.append({"source": f"Yahoo {symbole}", "articles": 0,
                        "erreur": str(exc)[:70]})
        return []
    ...
    clotures = ((bloc.get("indicators", {}).get("quote") or [{}])[0].get("close")) or []
    points = []
    for ts, valeur in zip(horodatages, clotures):
        if valeur is None:
            continue          # seance fermee ou donnee manquante
        points.append((datetime.fromtimestamp(ts, tz=timezone.utc).strftime("%Y-%m-%d"),
                       float(valeur)))
    journal.append({"source": f"Yahoo {symbole}", "articles": len(points),
                    "erreur": None if points else "aucune cotation renvoyee"})
    return points
...
def marquer_nouveautes(dossiers, memoire, jour):
    ...
    for d in dossiers:
        ...
        for entree in memoire:
            if proches(sac, set(entree["j"])):   # sans filtre : memoire hors lot
                connue = entree
                break
        if connue is None:
...
def main():
    ...
    if config.INSTRUMENTS:
        marche += etat_instruments(journal_marche)
    if not marche:
        print("  ATTENTION : aucune donnee de marche. La note perdra la lecture "
              "inversee et les divergences.", file=sys.stderr)
    ...
    for cle, theme in config.THEMES.items():
        print(f"{theme['titre']}...")
        journal = []
        brut = rss(theme["rss"], journal)
        if getattr(config, "GDELT_ACTIF", False):
            brut += gdelt(theme["gdelt"], journal)
        time.sleep(2)   # respirer entre les themes plutot que marteler GDELT
